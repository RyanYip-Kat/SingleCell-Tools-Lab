...
Rcpp::sourceCpp(code='
  ...
  Rcpp::NumericVector rowCorCpp(IntegerVector idxX, IntegerVector idxY, Rcpp::NumericMatrix X, Rcpp::NumericMatrix Y) {
    ...
    for (int i = 0; i < nx; ++i) {
      inv_sqrt_ss_X(i) = 1/sqrt(Rcpp::sum( X(Rcpp::_, i) * X(Rcpp::_, i) )+0.0001);
    }
    Rcpp::NumericVector inv_sqrt_ss_Y(ny);
    for (int i = 0; i < ny; ++i) {
      inv_sqrt_ss_Y(i) = 1/sqrt(Rcpp::sum( Y(Rcpp::_, i) * Y(Rcpp::_, i) )+0.0001);
    }
...
parser$add_argument("--scATAC",
                    type="character",
                    default="Aggregated scATAC Summarized Experimen")
parser$add_argument("--scRNA",
                    type="character",
                    default="Aggregated scRNA Summarized Experimen")

parser$add_argument("--gtf",
                    type="character",
                    default="gtf file from cellrange gtf")
#Input Files
...
#Input Files
scATAC_file <- args$scATAC
scRNA_file <- args$scRNA
gtf_file <- args$gtf
...
#mat=calculateCPM(seA,exprs_values="counts");mat@x=log2(mat@x/100+1);assay(seA)=mat
assay(seB) <- log2(edgeR::cpm(assay(seB))/100+1)
#mat=calculateCPM(seB,exprs_values="counts");mat@x=log2(mat@x/100+1);assay(seB)=mat
...
peakLinks$sigCorrelation <- abs(peakLinks$Correlation) >= corCutOff & peakLinks$FDR <= fdrCutOff & abs(peakLinks$distance) >= distCutOff
...
saveRDS(outMatch, file.path(args$outdir,"Save-P2G-Links.rds"))
...
idxCancer <- which(grepl("3h",colData(seA)$groupATAC)) #MPAL = MPAL Cells, RM = MPAL Cells from Ravi Majeti
idxHealthy <- which(!grepl("3h",colData(seA)$groupATAC)) #MPAL = MPAL Cells, RM = MPAL Cells from Ravi Majeti
...
o$PvalCancer <- pnorm((o$CorrelationCancer - mean(nullCorCancer[[2]])) / sd(nullCorCancer[[2]]))
o$PvalHealthy <- pnorm((o$CorrelationHealthy - mean(nullCorHealthy[[2]])) / sd(nullCorHealthy[[2]]))
...
mcols(peakLinks) <- mcolsLinks
peakLinks$peakName <- paste(seqnames(peakLinks), start(peakLinks), end(peakLinks), sep = "_")
peakLinks$sigCancer <- abs(peakLinks$CorrelationCancer) >= corCutOff & peakLinks$FDRCancer <= fdrCutOff & abs(peakLinks$distance) >= distCutOff
peakLinks$sigHealthy <- abs(peakLinks$CorrelationHealthy) >= corCutOff & peakLinks$FDRHealthy <= fdrCutOff & abs(peakLinks$distance) >= distCutOff
...
saveRDS(outMatch, file.path(args$outdir,"Save_MPAL_P2G_Links.rds"))
