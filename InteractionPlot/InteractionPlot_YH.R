...
bc<-subset(object,idents=c("4","16","17","21"))
tc<-subset(object,idents=c("1","7","3","24","8","10","19"))
mono=subset(object,idents=c("2","5","14","12"))

bc$celltype<-"BC"  # BC
tc$celltype<-"TC" # TC
mono$celltype<-"Mono"

seurat<-merge(x=bc,y=c(tc,mono),merge.data=TRUE)
...
seurat$status<-x
print(table(seurat$status))

Idents(seurat)<-seurat$status
seurat<-subset(seurat,idents="YH")
...
cell2cell<-c("Mono_TC","Mono_BC","TC_Mono","BC_Mono","TC_BC","BC_TC")
mat<-mat[,cell2cell]
...
write.table(mat,file.path(args$outdir,"InteractionScores.csv"),sep=",",quote=F)
