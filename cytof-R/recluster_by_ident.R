...
if(!is.null(args$ident)){
	seurat=subset(seurat,idents=args$ident,invert=args$invert)
}
...
seurat <- FindClusters(object = seurat,resolution=1.0, verbose =TRUE)
...
p<-DimPlot(seurat,reduction="umap",pt.size=2.0,label=T,label.size=7.5)+my_theme
print(p)
dev.off()
