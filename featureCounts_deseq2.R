...
parser$add_argument("-i", "--featurecount_file", type="character", default=NULL, help="Feature count file generated by the SubRead featureCounts command.")
parser$add_argument("-o", "--outdir", type="character", default='./', help="Output directory")
parser$add_argument("-g", "--group", type="character", default=NULL, help="Group file")
parser$add_argument("-e", "--excludeChr",nargs='+',type="character", default=NULL, help="exclude chroms")
parser$add_argument("-p", "--outprefix", type="character", default='differential', help="Output prefix")

parser$add_argument("-s", "--outsuffix", type="character", default='', help="Output suffix for comparison-level results")
parser$add_argument("-v", "--vst",action="store_true",help="Run vst transform instead of rlog")
parser$add_argument("-c", "--cores", type="integer", default=1, help="Number of cores")
...
if (is.null(opt$group)){
    #print_help(opt)
    stop("Please provide group file of featurecount file.", call.=FALSE)
}
...
if(!is.null(opt$excludeChr)){
        if(length(opt$excludeChr)>0){
                exclude=paste(opt$excludeChr,collapse="|")
        }
        cat(sprintf("### Exclude Chr Pattern : %s\n",exclude))
        chroms_idx=str_detect(count.table$Chr,exclude)
        count.table=count.table[!chroms_idx,]
}
...
print(colnames(count.table))

groups=as.character(read.table(opt$group,sep="\t",stringsAsFactors=FALSE)$V1)
print(unique(groups))
...
#groups=as.character(lapply(colnames(count.table), function (i)return(str_split(i,"\\.")[[1]][1])))
#print(unique(groups))
#if (length(unique(groups)) == 1) {
#    quit(save = "no", status = 0, runLast = FALSE)
#}
