...
jpeg(file.path(args$outdir,paste0(pathways.show,"_netVisual.jpeg")),width = 1024,height = 1024)
...
#ggsave(file.path(args$outdir,paste0(pathways.show,"_netVisual.pdf")),width = 16,height = 16,device =cairo_pdf)
dev.off()
...
cellchat <- identifyCommunicationPatterns(cellchat, pattern = "outgoing", k = nPatterns,width=16,height=10)
dev.off()

pdf(file.path(args$outdir,"incoming_Patterns.pdf"),width=8,height=16)
cellchat <- identifyCommunicationPatterns(cellchat, pattern = "incoming", k = nPatterns,width=16,height=10)
dev.off()
...
pdf(file.path(args$outdir,"outgoing_netAnalysis_dot.pdf"),width=8,height=8)
...
pdf(file.path(args$outdir,"incoming_netAnalysis_dot.pdf"),width=8,height=8)
