...
cols=c("Mono|CD4","Mono|CD8","Mono|Mono","CD8|CD4","CD8|CD8","CD8|Mono","CD4|CD4",
       "CD4|CD8","CD4|Mono")
means_path="cellphoedb-vkh/HC/out/means.txt"
pvalues_path="cellphoedb-vkh/HC/out/pvalues.txt"
...
h=cellphonedb_data_input(means_path = "cellphoedb-vkh/HC/out/means.txt",
                          pvalues_path ="cellphoedb-vkh/HC/out/pvalues.txt",
                          selected_rows = rows,
                          selected_columns =cols6)

v=cellphonedb_data_input(means_path = "cellphoedb-vkh/VKH/out/means.txt",
                           pvalues_path ="cellphoedb-vkh/VKH/out/pvalues.txt",
                           selected_rows = rows,
                           selected_columns = cols6)


write.table(h,"hc_cellphone6.csv",row.names = T,sep=",",quote = F)
write.table(v,"vkh_cellphone6.csv",row.names = T,sep=",",quote = F)
...
ggsave("plot6.pdf", width = 8, height = 24, device = cairo_pdf, limitsize=F)
dev.off()
...
col.heatmap <- colorRampPalette(c(col1,col2,col3 ))( 1000 )
pmat1=heatmaps_plot_data(meta_file = "cellphone_vkh/HC/cell_meta.txt",
                pvalues_file = "cellphone_vkh//HC/out/pvalues.txt")

pmat2=heatmaps_plot_data(meta_file = "cellphone_vkh/VKH/cell_meta.txt",
                 pvalues_file = "cellphone_vkh/VKH/out/pvalues.txt")
...
p1+p2  # common lengend
dev.off()




mat=read.csv("cellphone_vkh/heatmap_count.csv",stringsAsFactors = F,row.names = 1)
pdf("cellphone_vkh/heatmap.pdf", width = 8, height = 8)
pheatmap(mat[,c("HC_TC","VKH_TC","HC_Mono","VKH_Mono")],
         cluster_rows = F,
         cluster_cols = F,
         color = col.heatmap)
