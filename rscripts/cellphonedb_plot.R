...
rows=as.character(read.csv("../20200729/selected-cellphonedb.csv",sep = ",",header = F,stringsAsFactors = F)$V1)
rows=str_trim(rows,"right")
cols=c("Mono|CD4","Mono|CD8","Mono|Mono","CD8|CD4","CD8|CD8","CD8|Mono","CD4|CD4",
       "CD4|CD8","CD4|Mono")
means_path1="../20200727/DR-HC-Cellphonedb/DR/out/means.txt"
pvalues_path1="../20200727/DR-HC-Cellphonedb/DR/out/pvalues.txt"

means_path2="../20200727/DR-HC-Cellphonedb/HC/out/means.txt"
pvalues_path2="../20200727/DR-HC-Cellphonedb/HC/out/pvalues.txt"

dot_plot(means_path = means_path1,pvalues_path = pvalues_path1,selected_rows = rows)

data1=cellphonedb_data_input(selected_rows = rows,means_path = means_path1,pvalues_path = pvalues_path1, selected_columns=cols)
data2=cellphonedb_data_input(selected_rows = rows,means_path = means_path2,pvalues_path = pvalues_path2,selected_columns=cols)
...
p1=ggplot(data1,aes(x=clusters,y=pair,size=-log10(pvalue),color=mean)) +
  geom_point()+
...
p2=ggplot(data2,aes(x=clusters,y=pair,size=-log10(pvalue),color=mean)) +
  geom_point()+
  scale_color_gradientn('Log2 mean (Molecule 1, Molecule 2)', colors=my_palette) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
...
ggsave("plot.pdf", width = 18, height = 12, device = cairo_pdf, limitsize=F)
dev.off()


hc=cellphonedb_data_input(means_path = "../20200729/vkh_cellphonedb/hc/out/means.txt",
                          pvalues_path ="../20200729/vkh_cellphonedb/hc/out/pvalues.txt" )

vkh=cellphonedb_data_input(means_path = "../20200729/vkh_cellphonedb/vkh/out/means.txt",
                          pvalues_path ="../20200729/vkh_cellphonedb/vkh/out/pvalues.txt" )


commom_pair=intersect(hc$pair,vkh$pair)
...
h=cellphonedb_data_input(means_path = "../20200729/vkh_cellphonedb/hc/out/means.txt",
                          pvalues_path ="../20200729/vkh_cellphonedb/hc/out/pvalues.txt",
                          selected_rows = commom_pair,
                          selected_columns =cols4)

v=cellphonedb_data_input(means_path = "../20200729/vkh_cellphonedb/vkh/out/means.txt",
                           pvalues_path ="../20200729/vkh_cellphonedb/vkh/out/pvalues.txt",
                           selected_rows = commom_pair,
                           selected_columns = cols4)
...
ggsave("plot4.pdf", width = 6, height = 18, device = cairo_pdf, limitsize=F)
dev.off()
...
col.heatmap <- colorRampPalette(c(col1,col2,col3 ))( 1000 )
pmat1=heatmaps_plot_data(meta_file = "../20200729/vkh_cellphonedb/hc/out/cell_meta.txt",
                pvalues_file = "../20200729/vkh_cellphonedb/hc/out/pvalues.txt")

pmat2=heatmaps_plot_data(meta_file = "../20200729/vkh_cellphonedb/vkh/out/cell_meta.txt",
                 pvalues_file = "../20200729/vkh_cellphonedb/vkh/out/pvalues.txt")


write.table(pmat1,"heatmap_count_hc.csv",sep=",",quote = F)
write.table(pmat2,"heatmap_count_vkh.csv",sep=",",quote = F)
p1=Heatmap(pmat1,col = col.heatmap,name = "hc")
p2=Heatmap(pmat2,col = col.heatmap,name = "vkh")
pdf("heatmap.pdf", width = 24, height = 16)
p1+p2
dev.off()
