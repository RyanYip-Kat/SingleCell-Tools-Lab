...
if(!is.null(args$column) & !is.null(args$subset) & is.null(args$barcode)){
	...
	Data=Data[Data[[args$column]]%in%args$subset,,drop=FALSE]
	if(args$invert){
		Data=Data[!Data[[args$column]]%in%args$subset,,drop=FALSE]
	}
	print(paste0("After subset,Size of cells :",nrow(Data)))
	print(head(Data))
	...
	Data=Data[Data$barcode%in%cell,,drop=FALSE]
	if(args$invert){
		Data=Data[!Data$barcode%in%cell,,drop=FALSE]
	}
}

toc=toc[,Data$barcode]
print(paste0("Size of toc  [ ",nrow(toc),",",ncol(toc)," ]"))
genes<-rownames(toc)
