...
Cov.Heatmap<-function(x=NULL,genes=NULL,
                      ident=0,slot="data",type="lower",
                      label=FALSE,p.mat=FALSE,figure="."){
  if("Seurat"!=class(x)){
    stop("x must be a Seurat object!")
  }
  cells<-WhichCells(x,idents =ident)
  if (slot == "raw") {
    DATA <- x@assays$RNA@counts
  }
  if (slot== "data") {
  ...
  jpeg(paste0(figure,"/","cov.heatmap_",ident,".jpeg"),width=1024,height=1024)
  p<-ggcorrplot(cormat,method = "square",
                ...
                show.diag = TRUE,
                lab = label,
                p.mat = p.mat,insig = "pch")+theme_bw()+ggtitle(paste0("Cell :",ident))+
...
Cluster.FeaturePlot<-function(x,
			      cluster,
			      marker="CD4",
  ...
  for(i in 1:length(feature)){
      ...
      p<-FeaturePlot(x,features = marker,
                     cells = cells,
                     reduction = "tsne",
                     pt.size = 1.0)+ggtitle(label=feature[[i]])
  ...
  p2<-ggplot(data=expr.value,aes(x=group,y=expr,fill=group))+
    ...
    theme_bw()+NoGrid()+ggtitle(paste(cluster," (",marker,") "))+
...
require(NMF)
aheatmap.idents<-function(object,features,idents=NULL){
	if(is.null(idents)){
		x<-object
	}else{
		x<-subset(object,idents=idents)
	}

        data<-GetAssayData(x,slot="data")[features,Cells(x)]
	data<-as.matrix(data)
	annotation<-data.frame("celltype"=Idents(x),
			       "disease"=x@meta.data$disease)

	aheatmap(data, 
		cexRow=3.0,
		fontsize=20,
		annCol = annotation,
		filename="aheatmap.jpeg",
		width=1024,
		height=1024)
}


Ident.cv<-function(object,features,ident){
	x<-subset(object,idents=ident)
	data<-GetAssayData(x,slot="data")[features,Cells(x)]
	data<-as.matrix(data)
	cv<-apply(data,1,function(z){
			  m<-mean(z)
			  s<-sd(z)
			  cv<-s/(m+1e-3)
			  return(cv)
