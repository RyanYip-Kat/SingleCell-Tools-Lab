...
parser$add_argument("--se",
		    type="character",
		    default=NULL,
		    help="PeakSet scATAC SummarizedExperiment Object")
...
parser$add_argument("--trajectory",
		    ...
		    nargs="+",
                    default=NULL,
		    help="the groups in se colData used as trajectory aligned")

parser$add_argument("--groupby",
                    type="character",
                    default="label_fine",
		    help="the group used as trajectory aligned")
...
# Get Inputs
#----------------------------
message("INFO : Loading scATAC Summarized-Experiment data")
se <- readRDS(args$se)
Group=colData(se)[[args$groupby]]
#trajectory <- paste0("C",c(2,3,4,1,6,5))
cat(sprintf("INFO : trajectory aligned with Clusters : %s\n",paste(args$trajectory,collapse=",")))
trajectory<-args$trajectory


message("INFO : Align single cells to Trajectory")
#Align single cells to Trajectory
df <- data.frame(row.names = colnames(se), x = colData(se)$UMAP1, y = colData(se)$UMAP2, Group = Group)
...
p=ggplot(df2, aes(x,y,color=pseudotime)) + geom_point() +
...
ggsave(filename=file.path(args$outdir,"UMAP-PseudoTime.pdf"),plot=p,width=12,height=16,device =cairo_pdf)

saveRDS(trajAligned, file.path(args$outdir,"Aligned-Trajectory.rds"))

#Significance of Trajectory Order
message("INFO : Significance of Trajectory Order")
clustMeans <- edgeR::cpm(groupSums(assay(se), Group, sparse = TRUE),log=TRUE,prior.count=3)
...
p=ggplot(rankDF, aes(t,ranks,color=names)) +
    geom_point(size = 8) +
...
ggsave(filename=file.path(args$outdir,"Trajectory_Plot_Distance.pdf"),plot=p,width=12,height=16,device =cairo_pdf)

out <- list(trajStats = trajStats, rankDF = rankDF, rankV = rankV)
saveRDS(out, file.path(args$outdir,"Trajectory-Stats.rds"))
