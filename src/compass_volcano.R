...
df=compass_scores_by_cell_type[,cols]
write.table(df,file.path(args$outdir,"compass_scores_by_cell_df.csv"),sep=",",quote=T,row.names=F)
write.table(compass_scores_by_cell_type,file.path(args$outdir,"compass_scores_by_cell.csv"),sep=",",quote=T,row.names=F)
ggplot(
    compass_scores_by_cell_type,
    aes(
        x = cohens_d,
...
ggtitle("Differential COMPASS Scores for NPDR vs. DME( CD14 Cells)") +
...
theme_bw()
ggsave(file.path(args$outdir,"compass_plot.pdf"),width = 16,height = 16,device =cairo_pdf)
dev.off()
...
write.table(df,file.path(args$outdir,"compass_scores_by_cell_df.csv"),sep=",",quote=T,row.names=F)
write.table(compass_scores_by_cell_type,file.path(args$outdir,"compass_scores_by_cell.csv"),sep=",",quote=T,row.names=F)
saveRDS(compass_scores_by_cell_type,file.path(args$outdir,"compass_scores_by_cell.rds"))
ggplot(
    compass_scores_by_cell_type,
    aes(
        x = cohens_d,
...
theme_bw()
ggsave(file.path(args$outdir,"compass_plot_all.pdf"),width = 16,height = 16,device =cairo_pdf)
dev.off()
