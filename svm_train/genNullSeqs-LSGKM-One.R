...
genNullSeqs = function(
  ...
  genome = NULL  # if genome is provided, genome version is ignored 
    ...
    #xfold = 1; 
    if(is.null(genome)){  
      if(toupper(genomeVersion)=='HG38'){
        if(requireNamespace("BSgenome.Hsapiens.UCSC.hg38.masked", quietly = TRUE)){
...
genNullSeqs(bedfile,nMaxTrials=10,
            xfold=1,
            genomeVersion='hg38',
            outputPosFastaFN=file.path(output,'ctcfpos.fa'),
            outputBedFN=file.path(output,'ctcf.bed'),
            outputNegFastaFN=file.path(output,'ctcfneg.fa'))


print("INFO : filter fasta")
pattern="N"
pos=file.path(output,'ctcfpos.fa')
cmd=paste("python","/home/ye/Work/BioAligment/SNP/Shi/svm_train/filter_fasta.py",pos,output,"--pattern",pattern,collapse=" ")
system(cmd)

neg=file.path(output,'ctcfneg.fa')
cmd=paste("python","/home/ye/Work/BioAligment/SNP/Shi/svm_train/filter_fasta.py",neg,output,"--pattern",pattern,collapse=" ")
system(cmd)
