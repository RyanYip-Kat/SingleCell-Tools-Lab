write_src="/home/ye/Work/R/Cytof/cellranger/write10xCounts_h5_from_seurat.R"
...
do
	...
	seurat=${arr[1]}
	output=${out}/$sample
	$rscript $write_src --seurat $seurat --outdir $output
	cd $output/matrix  && awk '{print $1"\t"$2"\tGene Expression"}' genes.tsv > features.tsv && rm genes.tsv && sed -i 's/\./\-/' barcodes.tsv && gzip *
	cd -
