...
Version 1.1, released November 6, 2019

1.1 - fixed occasional infinite loop bug during post-processing
    ...
    for contig, fragment_list in group_fragments_by_start(filename, index):
        ...
        for _, start, stop, barcode in fragment_list:
            ...
            if len(stops) > MAXIMUM_STOP_DICT_SIZE:
                look_up = {x: None for x in stops.keys()}
                for key in look_up:
...
def main(args):
    ...
    singlecell_df = pd.read_csv(args.singlecell)
    ...
    for barcode, linked_barcode, count in iterate_through_fragment_adjacencies(
        args.fragments, args.fragments_index, logout, starttime
    ):
    ...
    temp_lookup = {x: None for x in putative_barcode_multiplets.keys()}
    for minor_barcode in temp_lookup.keys():
        if minor_barcode not in putative_barcode_multiplets:
    ...
    for minor_barcode, major_barcode in putative_barcode_multiplets.items():
        if major_barcode in putative_barcode_multiplets:
    ...
    with open(args.excluded_barcodes, "w") as excluded_out:
        excluded_out.write(
            "{},{},{}\n".format(
