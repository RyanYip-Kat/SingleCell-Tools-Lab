...
rowData(se)=all_Peaks
peak=paste(all_Peaks$seqnames,all_Peaks$start,all_Peaks$end,sep="-")
rownames(se)=peak
mat=assay(se)
if(args$binarize){
...
for(name in names(markerList)){
        fragmentsj <- markerList[[name]]
	peak=paste(seqnames(fragmentsj),start(fragmentsj),end(fragmentsj),sep="-")
	clustesum=clusterSums[peak,name]
	out=data.frame("chr"=seqnames(fragmentsj),
		       "start"=start(fragmentsj),
		       "end"=end(fragmentsj),
		       "count"=clustesum)
	path=file.path(args$outdir, paste0(str_replace(name," ","-"), "_featureCount.txt"))
